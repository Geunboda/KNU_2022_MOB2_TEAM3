    ...
    var body: some View {
        VStack(spacing: 35) {
            ...
            VStack(spacing: 15) {
                ...
                if let schedules = schedules.filter { schedule in
        ...
        VStack {
            if value.day != -1 {
                if let schedule = schedules.first(where: { schedule in
                    return isSameDay(date1: schedule.startDate!, date2: value.date )
                }) {
                    Text("\(value.day)")
                        .font(.title3.bold())
                        .foregroundColor(isSameDay(date1: schedule.startDate!, date2: currentDate) ? .white : .primary)
                        .frame(maxWidth: .infinity)
                    Spacer()
                    Circle()
                        .fill(isSameDay(date1: schedule.startDate!, date2: currentDate) ? .white : Color("Blue"))
